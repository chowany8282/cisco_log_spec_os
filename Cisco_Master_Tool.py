...
tab0, tab1, tab2, tab3 = st.tabs(["🚨 로그 선별 (AI)", "📊 로그 정밀 분석", "🔍 하드웨어 스펙", "💿 OS 추천"])

# ========================================================
# [TAB 0] 로그 선별기 (헤더 삭제 + 타임스탬프 필터링)
# ========================================================
with tab0:
    st.header("⚡ 로그 선별 (Timestamp Filter)")
    st.caption("AI가 날짜/시간이 없는 불필요한 텍스트를 제거하고, **유효한 로그만 추출**해 줍니다.")
    ...
    if run_btn:
        ...
        if not final_log_content:
            ...
            with st.spinner("AI가 유효한 로그만 골라내는 중..."):
                # [수정됨] Critical/Warning 분류 헤더 삭제 요청
                prompt = f"""
                당신은 시스코 로그 정제 전문가입니다. 
                제공된 텍스트에서 **날짜와 시간(Timestamp)이 포함된 유효한 로그**만 추출하세요.

                [엄격한 필터링 규칙]
                1. **날짜/시간이 없는 라인은 무조건 버리세요.** (예: 'server status:', 'User Name:', 명령어 입력줄 등 삭제)
                2. 유효한 로그 예시: `Jan 31 21:03:03 ...` 또는 `2024/01/01 12:00 ...` 또는 `*Feb 1 ...`
                3. **Critical, Warning 같은 분류 제목(Header)을 붙이지 마세요.** 그냥 번호 매겨서 나열하세요.
